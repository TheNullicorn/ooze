...
public class BitCompactIntArray implements IntArray, OozeSerializable {
    ...
    /**
     * Performs the {@link #set(int, int)} operation on a compact int array independent of its
     * {@link BitCompactIntArray wrapper} object. This allows new arrays to be modified directly,
     * such as when resizing.
     */
    private static int setInternal(byte[] raw, int bitsPerCell, int cellMask, int index, int value) {
        ...
        while (valueMask != 0) {
            ...
            previousValue |= (((raw[byteIndex] & 0xFF) >> bitOffset) & valueMask)
                             << totalBitsWritten;
    ...
    /**
     * Changes the maximum allowed value to be inserted into the array. If the {@code newMaxValue}
     * is less than the previous {@link #maxValue() maxValue}, it is expected that all values in the
     * array are already less than the new maximum.
     *
     * @throws IllegalStateException If the array contains values greater than the new maximum
     *                               value.
     */
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (value > newMaxValue) {
                    throw new IllegalStateException(
                        "Cannot change maximum value to " + newMaxValue +
    ...
    public void serialize(DataOutputStream out) throws IOException {
        ...
        oozeOut.writeVarInt(maxValue);
        oozeOut.write(data);
    }

    /**
     * Changes the size of the internal array so that is only as large as is needed to store values
     * up to {@code newMaxValue}. If this change causes the size to decrease, it is expected that
     * the array does not contain any values higher than the new maximum.
     * <p>
    ...
    private void resize(int newMaxValue) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (value > newMaxValue) {
                throw new IllegalStateException(
                    "Source array contains values larger than new maximum (" +
                    newMaxValue + "): " + value);
            }
