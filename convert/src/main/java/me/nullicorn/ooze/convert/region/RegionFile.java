...
import me.nullicorn.nedit.filter.FilteredNBTInputStream;
...
public class RegionFile implements ChunkSource {
  ...
  private static final NBTFilter chunkTagFilter = NBTFilter.with(
      "DataVersion",
      "Level.xPos",
      "Level.zPos",
      "Level.Entities",
  ...
  private NBTCompound deserializeChunkData(byte[] data, int offset, int length, int compressionType)
      throws IOException {
    InputStream rawIn = new ByteArrayInputStream(data, offset, length);
    ...
    if (compressionType == 2) {
      ...
      rawIn = new InflaterInputStream(rawIn);
    }

    FilteredNBTInputStream chunkIn = new FilteredNBTInputStream(rawIn, true, true);
    return chunkIn.readFully(chunkTagFilter);
  }
